...
void lapic_init(void) {
    ...
    if (ret != 0) {
        log_early("error: Failed to map LAPIC 0x%lx to 0x%lx (%d)", phys_addr,
                  virt_addr, ret);
...
void lapic_send_ipi(uint32_t apic_id, uint32_t vector, uint32_t delivery_mode,
    ...
    if (vector > 255 && delivery_mode != ICR_INIT &&
        delivery_mode != ICR_STARTUP) {
    ...
    uint32_t icr_lo =
        vector | delivery_mode | dest_mode | ICR_ASSERT | ICR_EDGE | shorthand;
