...
void madt_init() {
    ...
    while (offset < madt->sdt.length - sizeof(acpi_madt_t)) {
        ...
        if (entry->length == 0 ||
            offset + entry->length > madt->sdt.length - sizeof(acpi_madt_t)) {
        ...
        case MADT_ENTRY_LAPIC: // Type 0: Local APIC
        {
            ...
            log_early(
                "MADT: Found Local APIC, proc_id=%u, apic_id=%u, flags=0x%x",
        ...
        case MADT_ENTRY_IOAPIC: // Type 1: I/O APIC
        {
            ...
            log_early(
                "MADT: Found I/O APIC, ioapic_id=%u, addr=0x%x, gsi_base=%u",
        ...
        case MADT_ENTRY_IOAPIC_SRC_OVR: // Type 2: Interrupt Source Override
        {
            acpi_madt_ioapic_src_ovr_t* iso =
                (acpi_madt_ioapic_src_ovr_t*)entry;
            ...
            log_early(
                "MADT: Found I/O APIC ISO, bus=%u, irq=%u, gsi=%u, flags=0x%x",
                iso->bus_source, iso->irq_source, iso->gsi, iso->flags);
            break;
        }
        case MADT_ENTRY_IOAPIC_NMI_SRC: // Type 3: I/O APIC NMI Source
        {
            acpi_madt_ioapic_nmi_src_t* nmi_src =
                (acpi_madt_ioapic_nmi_src_t*)entry;
            ...
            log_early("MADT: Found I/O APIC NMI Source, nmi_source=%u, gsi=%u, "
                      "flags=0x%x",
        ...
        case MADT_ENTRY_LAPIC_NMI: // Type 4: Local APIC NMI
        {
            ...
            log_early(
                "MADT: Found Local APIC NMI, proc_id=%u, lint=%u, flags=0x%x",
                lapic_nmi->acpi_proc_id, lapic_nmi->lint, lapic_nmi->flags);
            break;
        }
        case MADT_ENTRY_LAPIC_ADDR_OVR: // Type 5: LAPIC Address Override
        {
            acpi_madt_lapic_addr_ovr_t* addr_ovr =
                (acpi_madt_lapic_addr_ovr_t*)entry;
        ...
        case MADT_ENTRY_LX2APIC: // Type 9: Local x2APIC
        {
            ...
            log_early("MADT: Found Local x2APIC, x2apic_id=%u, acpi_id=%u, "
                      "flags=0x%x",
    ...
    log_early("MADT parsed: %u LAPIC(s), %u IOAPIC(s), %u ISO(s), %u LAPIC "
              "NMI(s), %u IOAPIC NMI(s), %u x2APIC(s), LAPIC addr: 0x%lx",
