...
void acpi_init(void) {
    ...
    acpi_rsdp_t* rsdp =
        (acpi_rsdp_t*)vallocat(kvm_ctx, 1, VALLOC_RW, rsdp_response->address);
...
void* acpi_find_table(const char* name) {
    if (!acpi_uses_xsdt) {
        ...
        for (uint32_t i = 0; i < entries; i++) {
            acpi_sdt_header_t* sdt =
                (acpi_sdt_header_t*)HIGHER_HALF(*((uint32_t*)rsdt->table + i));
    ...
    for (uint32_t i = 0; i < entries; i++) {
        acpi_sdt_header_t* sdt =
            (acpi_sdt_header_t*)HIGHER_HALF(*((uint64_t*)xsdt->table + i));
        if (!memcmp(sdt->signature, name, 4)) {
            return (void*)sdt;
        }
    }
